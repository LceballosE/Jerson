...
<div class="bgded overlay" style="background-image:url('images/mazos/uther.jpeg');">
  ...
  <main class="hoc container clear">
    ...
    <div class="group">
      <div class="one_half first">
         <div style="background:rgba(0,0,0,0.5);padding: 50px; border-radius: 20px" class="jumbotron">
        <h6 class="heading font-x3">La Justicia Exige Castigo</h6>
        <p style="text-align: justify;">Ultricies vel vestibulum quis sodales nec sem aliquam non ligula sed dui purus blandit ut viverra at bibendum id urna duis est elit condimentum ac venenatis sed congue ac ipsum aliquam erat elit commodo non hendrerit id vehicula</p>
      </div>
    </div>
  ...
  <section class="hoc container clear">
    ...
    <br><h2>Habilidades</h2></br>   
		...
		@foreach ($cartas as $carta)
					@if ($count>0)
			     <article class="one_quarter"><img src="{{asset($carta->imagen)}}" alt="">
			        <div class="txtwrap">
			          <h4 class="heading">{{$carta->nombre}}</h4>
			          <p>CLASE: {{$carta->nclase}}</p>
			           <p>TIPO: {{$carta->ntipo}}</p>
			            <p>ATAQUE: {{$carta->ataque}}</p>
			             <p>VIDA: {{$carta->vida}}</p>
                   <p>EXPANSIÓN: {{$carta->nexpansion}}</p>
                    <p>RAREZA: {{$carta->nrareza}}</p>
			       		</div>
			   		</article>	
			   		<?php $count=$count+1;?>
			   	...
			   	@endif
          @if ($count==0)
			   	 <article class="one_quarter first"><img src="{{asset($carta->imagen)}}" alt="">
			        <div class="txtwrap">
			          <h4 class="heading">{{$carta->nombre}}</h4>
			         <p>CLASE: {{$carta->nclase}}</p>
                 <p>TIPO: {{$carta->ntipo}}</p>
